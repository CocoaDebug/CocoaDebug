...
NSString *const _GPBWellKnownTypesErrorDomain =
    _GPBNSStringifySymbol(_GPBWellKnownTypesErrorDomain);
...
@implementation _GPBTimestamp (GBPWellKnownTypes)
...
@implementation _GPBDuration (GBPWellKnownTypes)
...
@implementation _GPBAny (GBPWellKnownTypes)

+ (instancetype)anyWithMessage:(_GPBMessage *)message
...
- (instancetype)initWithMessage:(_GPBMessage *)message
                          error:(NSError **)errorPtr {
...
- (BOOL)packWithMessage:(_GPBMessage *)message
                  error:(NSError **)errorPtr {
...
- (BOOL)packWithMessage:(_GPBMessage *)message
  ...
  if (fullName.length == 0) {
    if (errorPtr) {
      *errorPtr =
          [NSError errorWithDomain:_GPBWellKnownTypesErrorDomain
                              code:_GPBWellKnownTypesErrorCodeFailedToComputeTypeURL
                          userInfo:nil];
    }
    return NO;
  }
...
- (_GPBMessage *)unpackMessageClass:(Class)messageClass
                             error:(NSError **)errorPtr {
  ...
  if ((expectedFullName == nil) || ![expectedFullName isEqual:fullName]) {
    if (errorPtr) {
      *errorPtr =
          [NSError errorWithDomain:_GPBWellKnownTypesErrorDomain
                              code:_GPBWellKnownTypesErrorCodeTypeURLMismatch
                          userInfo:nil];
    }
    return nil;
  }
