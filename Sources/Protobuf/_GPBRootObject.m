...
// _GPBMessage is arguably the more "obvious" choice, but given that all messages
// inherit from _GPBMessage, conflicts seem likely, so we are using _GPBRootObject
// instead.
@interface _GPBRootObject () <_GPBMessageSignatureProtocol>
@end

@implementation _GPBRootObject
...
static const void *_GPBRootExtensionKeyRetain(CFAllocatorRef allocator,
                                             const void *value) {
#pragma unused(allocator)
  return value;
}

static void _GPBRootExtensionKeyRelease(CFAllocatorRef allocator,
                                       const void *value) {
#pragma unused(allocator)
#pragma unused(value)
}

static CFStringRef _GPBRootExtensionCopyKeyDescription(const void *value) {
  const char *key = (const char *)value;
  return CFStringCreateWithCString(kCFAllocatorDefault, key,
                                   kCFStringEncodingUTF8);
}

static Boolean _GPBRootExtensionKeyEqual(const void *value1,
                                        const void *value2) {
...
static CFHashCode _GPBRootExtensionKeyHash(const void *value) {
  const char *key = (const char *)value;
...
static _GPBExtensionRegistry *gDefaultExtensionRegistry = NULL;
...
+ (void)initialize {
  ...
  if (!gExtensionSingletonDictionary) {
    ...
    CFDictionaryKeyCallBacks keyCallBacks = {
      ...
      0,
      _GPBRootExtensionKeyRetain,
      _GPBRootExtensionKeyRelease,
      _GPBRootExtensionCopyKeyDescription,
      _GPBRootExtensionKeyEqual,
      _GPBRootExtensionKeyHash,
    };
    gExtensionSingletonDictionary =
        CFDictionaryCreateMutable(kCFAllocatorDefault, 0, &keyCallBacks,
                                  &kCFTypeDictionaryValueCallBacks);
    gDefaultExtensionRegistry = [[_GPBExtensionRegistry alloc] init];
  }

  if ([self superclass] == [_GPBRootObject class]) {
...
BOOL _GPBResolveExtensionClassMethod(Class self, SEL sel) {
  ...
  if (extension != nil) {
    const char *encoding =
        _GPBMessageEncodingForSelector(@selector(getClassValue), NO);
...
#pragma unused(obj)
    ...
    if (!methodAdded) {
      methodAdded = _GPBClassHasSel(metaClass, sel);
    }
    return methodAdded;
  }
  return NO;
...
+ (BOOL)resolveClassMethod:(SEL)sel {
  if (_GPBResolveExtensionClassMethod(self, sel)) {
    return YES;
  }
