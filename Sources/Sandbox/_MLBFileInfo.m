...
- (instancetype)initWithFileURL:(NSURL *)URL {
    if (self = [super init]) {
        ...
        if ([self.attributes.fileType isEqualToString:NSFileTypeDirectory]) {
            ...
            //liman
            if ([URL isFileURL]) {
                self.modificationDateText = [NSString stringWithFormat:@"[%@] %@", [_SandboxHelper fileModificationDateTextWithDate:self.attributes.fileModificationDate], [_SandboxHelper sizeOfFolder:URL.path]];
            }
        } else {
            ...
            //liman
            if ([URL isFileURL]) {
                self.modificationDateText = [NSString stringWithFormat:@"[%@] %@", [_SandboxHelper fileModificationDateTextWithDate:self.attributes.fileModificationDate], [_SandboxHelper sizeOfFile:URL.path]];
            }
        }
...
+ (NSMutableArray<_MLBFileInfo *> *)sortedMLBFileInfoArray:(NSMutableArray<_MLBFileInfo *> *)array
{
    return [[[array copy] sortedArrayUsingComparator:^NSComparisonResult(_MLBFileInfo  *_Nonnull obj1, _MLBFileInfo  *_Nonnull obj2) {
    ...
    if (isExists && isDir) {
        ...
        if (!error) {
            for (NSString *name in contents) {
                if (_Sandbox.shared.isSystemFilesHidden && [name hasPrefix:@"."]) { continue; }
