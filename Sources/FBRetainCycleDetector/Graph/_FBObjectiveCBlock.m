...
@implementation _FBObjectiveCBlock
...
- (NSSet *)allRetainedObjects
{
  ...
  NSArray *allRetainedReferences = _FBGetBlockStrongReferences(blockObjectReference);

  for (id object in allRetainedReferences) {
    _FBObjectiveCGraphElement *element = _FBWrapObjectGraphElement(self, object, self.configuration);
    if (element) {
...
- (NSString *)classNameOrNull
{
  ...
  __attribute__((objc_precise_lifetime)) id anObject = self.object;
  if ([anObject isKindOfClass:[_FBBlockStrongRelationDetector class]]) {
    _FBBlockStrongRelationDetector *blockObject = anObject;
